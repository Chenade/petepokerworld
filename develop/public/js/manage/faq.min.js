$(document).ready(function(){
    ...
    var uploadImage = function(id){
        ...
        ajax('GET', '/api/image/' + tid + '/' + id + '/get', {}, function (a) {
            if(a.success){
                $('#display-img').empty();
                a.data.forEach(el => {
                    var box = '<div class="col-10 col-lg-3 d-flex flex-column align-items-center" style="margin: 10px; padding: 10px; border-radius: 10px; background-color: #e5e5e5;">';
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        ajax('GET', '/api/' + tid + '/'+id, {}, function (a) {
            if(a.success){
                $('#edit_title').val(a.data.question);
                $('#edit_content').val(a.data.answer.substr(1,a.data.answer.length -2));
    ...
    $(document).on('click', '.save-btn', function () {
        var data = {
            'question': $('#edit_title').val(),
            'answer': $('#edit_content').val(),
            'ord': $('#edit_ord').val(),
