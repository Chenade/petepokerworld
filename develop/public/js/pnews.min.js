$(document).ready(function() {
    ...
    //     </div>
    // </div>
    ajax('GET', '/api/news/list', {}, function (a) {
        $('#events').empty();
        $('#blog').empty();
        var list = a.data;
        for (var i in list) {
            var news = list[i];
            var div = '' +
                    '<div class="col-12 col-lg-4" style="padding: 25px;">' +
                        '<div class="service-box" style="box-shadow:-0.5px 2px 10px 1px rgba(56, 142, 144, 0.17);">' +
                            '<div class="col-12 post-thumbnail"><img src="/upload/Image/'+ news['image'] + '" alt="" ></div> ' +
                            '<div class="col-12" style="padding-left: 2em;">' +
                                '<div class="service-tag-title">' + news['title'] + '</div>' +
                                '<div class="service-detail">' + news['content'].substring(1, 150) + '. . .</div>' +
                            '<button class="btn news-detail-btn col-12" data-id="' + news['id'] + '" style="margin: 30px 0 0 0;">View Details <i class="fas fa-long-arrow-alt-right"></i></button>' +
                            '</div>\n' +
                        '</div>' +
                    '</div>';
            if (news['category'] == 1)
